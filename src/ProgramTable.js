import { ageStr, ageStr2, currentMonth, monthStr, monthInRange, ordinal } from './util';
...
export function CardView2() {
    ...
    const cards = orgs().map(org => {
        return <Card key={hash(org)}>
            <Card.Body>
                <Card.Title><a href={org.url}>{org.name}</a></Card.Title>
                <Card.Text>
                    Some intro text about the organization.
                </Card.Text>
                <h5>Programs</h5>
                <ListGroup variant="flush">
                    {org.programs.map(program => {
                        const prog = program;
                        return <ListGroup.Item variant={prog.registration && monthInRange(m, [prog.registration, prog.season[0]]) ? "warning" : ""} className="d-flex align-items-start justify-content-between">
                            <Col className="d-flex align-items-start justify-content-start">
                                {
                                    program.url ? <a href={program.url}>{program.name}</a> :
                                        program.name
                                }
                            </Col>
                            <Col className="d-flex align-items-start justify-content-end">
                                <Col className="d-flex justify-content-end">
                                    <div>{program.season ? `${monthStr(program.season[0])}–${monthStr(program.season[1])}` : ''}</div>
                                </Col>
                                <Col className="d-flex justify-content-end">
    ...
    return <div class="row row-cols-1 row-cols-md-2 row-cols-xl-3 g-4">
        {cards.map(c => <Col>{c}</Col>)}
    </div>
}
...
function AgeBadge({ program }) {
    ...
    if (allAges) {
        return <Badge pill bg="success">All</Badge>
    }
    ...
    //return <Badge pill bg="primary">{ret}</Badge>
    return <Badge pill bg="info">{ret}</Badge>
}
